...
All six phases described below have been implemented. This document is kept as a reference for the decisions made.
...
**Agent brief.** Added a lightweight perf probe to `index.html` that samples `performance.now()` around `buildFramePoints`, `drawEdges`, `drawNodes`, `drawClusterHalos`, and the d3 tick callback. Accumulated rolling 1-second averages, exposed them at `window.__perf` and painted them (opt-in via `?perf=1` query param) as a small HUD in the corner. Also logged `nodes.length`, `links.length`, `particlePool.length`, and `sim.alpha()`.

**Outcome.** With `?perf=1` on a synthetic 35k/120k graph, HUD shows per-section ms and FPS. Without the flag, zero overhead.
...
Replaced `graphHash(graph)` (which sorts and joins every node + link as a string) with a rolling hash: `nodes.length ^ links.length ^ (rolling xor of first/last 16 node ids) ^ sum(link source/target hashCodes)`. Single pass, no allocation. Feature-flagged it behind `cfg.fastHash` so we can A/B.

### 1b. Freeze simulation on convergence
When `sim.alpha() < 0.02` for 30 consecutive frames, called `sim.stop()`. Restarted (`sim.alpha(0.3).restart()`) only when `loadGraph` detects new nodes or links. Kept `forceCluster` gated on sim running, since it currently runs on every tick forever.

### 1c. Kill the per-frame sort in `drawNodes`
Removed the `[...nodes].sort(...)` copy entirely. Replaced with a two-pass draw: pass 1 draws non-ghost / low-importance (< 0.3), pass 2 draws high-importance + flared. No allocation, same z-order effect.

### 1d. Add `ultra` tier to `buildSceneTuning`
Current thresholds end at `massive: nodes > 3000`. Added:
...
**Outcome.** On a 35k/120k synthetic graph with "Ambient" preset: `graphHash` drops below 5ms (from ~200ms), steady-state frame time in `drawNodes` drops ≥20%, sim stops ticking within 3s of load.
...
Pre-rendered one 128×128 offscreen canvas containing a soft radial-gradient glow in white. In `drawNodes`, replaced the per-node `createRadialGradient` with `ctx.drawImage(glowSprite, x - r, y - r, size, size)` plus a tinted multiply (use `globalCompositeOperation = 'lighter'` or pre-tint into per-accent sprites cached in a Map keyed by color hex). This is the single biggest win — radial gradients are the hottest call in the profiler.

### 2b. Batch draws by color
In both `drawNodes` and `drawEdges`, grouped by color first, issued one `strokeStyle`/`fillStyle` + one `beginPath` per color group, then stroked/filled once. Canvas2D state changes cost ~1µs each; at 35k nodes that's 35ms of pure overhead we can reclaim.

### 2c. Static layer on OffscreenCanvas
`drawBackgroundGradient` + `drawClusterHalos` only change when (a) window resizes, (b) sim is running and moves meaningfully, or (c) config changes. Rendered them into an `OffscreenCanvas` once, `drawImage`ing it each frame. Invalidated the cache when sim restarts, and throttled halo re-render to once every 500ms max while sim runs.

### 2d. Particle pool pre-allocation
Replaced `push`/`splice` with a fixed-size array of `particleCap` slots and an `active: bool` flag per slot. Iterate slots, skip inactive. Eliminates GC pressure from the particle system.

**Outcome.** "Ambient" on 35k/120k holds ≥30fps with glow enabled. Frame time for `drawNodes` drops ≥50% vs Phase 1 baseline.
...
Rebuilt on sim settle or every N seconds while running. Used d3-quadtree (already in vendor'd d3). Stored in `sceneTuning.quadtree`.

### 3b. Viewport cull
Before `drawNodes` / `drawEdges`, computed visible-rect (with ~100px margin) and queried the quadtree. Edges are culled if *both* endpoints are outside. Skipped drawing + flare/label computation for culled nodes. Most 35k vaults have the majority of the graph offscreen at any moment — this should roughly halve draw work on typical aspect ratios.
...
**Outcome.** "Neon" (the most expensive preset) holds ≥30fps on 35k/120k with `maxRenderedNodes: 5000`. Lowering it to 2000 yields ≥50fps.
...
**Outcome.** Scrolling the settings page during a 35k graph load doesn't stutter. Calm-scene CPU usage drops ≥40%.
...
**Agent brief.** Added a "Dense" preset with:
- `motionMode: 'light'`
- `particles: false`, `noteFlare: false`
- `clusterHalos: false`, `backgroundGradient: true` (cached, cheap)
- `nodeGlow: true` but visually tiny — introduced a new cfg field `glowIntensity` (0–1, default 1) and let Dense use 0.35. The glow sprite system from Phase 2a makes this a one-line change.
...
1. **Audit the existing presets.** For each of the 8, captured a screenshot on a small (~200 node), medium (~2000), and large (~35000) synthetic vault. Noted what's visually distinct vs. redundant. My read: Classic, Minimal, Monochrome, and Embedded are near-duplicates; Ambient and Galaxy are near-duplicates.

2. **Define theme variance axes.** Proposed 4–5 orthogonal axes that a preset can vary along, e.g.:
...
4. **Added a theme-level `glowIntensity` and `edgeStyle` ('line' | 'curve' | 'none').** Unlocks the Constellation and Topographic looks without forking the render path per theme.

5. **Shipped a theme picker with screenshots** in settings.html. The current dot-only picker gives no preview of what the user is about to apply.

**Outcome.** 10 presets, each passing a blind "which one is this" test against screenshots of the other 9. Every preset holds ≥30fps on a 10k-node vault; Dense + Plain hold ≥30fps on 35k.
